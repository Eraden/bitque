...
pub fn load() -> Context {
  ...
  patterns: vec![
    ...
    Pattern::Match(MatchPattern {
      has_captures: false,
      regex: Regex::new("\\boperator\\s*(?:[-+*/%ˆ&|~!=<>]|[-+*/%^&|=!<>]=|<<=?|>>=?|&&|\\|\\||\\+\\+|--|,|->\\*?|\\(\\)|\\[\\]|\"\"\\s*\\b[\\p{L}_][\\p{L}\\p{N}_]*\\b)(?=\\s*(\\(|(?m:$)))"),
      scope: vec![
        Scope {
            a: 46444084125499392,
            b: 0,
    ...
    Pattern::Match(MatchPattern {
      ...
      operation: MatchOperation::Push(vec![
        ContextReference::Direct(ContextId { index: 3585 }),
    ]),
      with_prototype: None
    }),
    Pattern::Match(MatchPattern {
      has_captures: false,
      regex: Regex::new("(?=(?:::\\s*)?(?:\\b[\\p{L}_][\\p{L}\\p{N}_]*\\b\\s*::\\s*)*(?:template\\s+)?\\b[\\p{L}_][\\p{L}\\p{N}_]*\\b(<(?:[^(){}&;*^%=<>-]+(?:<(?:[^(){}&;*^%=<>-]+(?:<[^(){}&;*^%=<>-]*>)?)?\\s*>)?)?[^(){}&;*^%=<>-]*(?:\\([^(){}&;*^%=<>-]*\\))?[^(){}&;*^%=<>-]*>)\\s*(\\())"),
      scope: vec![],
      captures: None,
      operation: MatchOperation::Set(vec![
        ContextReference::Direct(ContextId { index: 3762 }),
        ContextReference::Direct(ContextId { index: 3719 }),
    ]),
      with_prototype: None
    }),
    Pattern::Match(MatchPattern {
      ...
      operation: MatchOperation::Set(vec![
        ContextReference::Direct(ContextId { index: 3762 }),
        ContextReference::Direct(ContextId { index: 3718 }),
    ]),
      with_prototype: None
    }),
    Pattern::Match(MatchPattern {
