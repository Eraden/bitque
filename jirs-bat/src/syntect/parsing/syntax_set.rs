...
use crate::syntect::parsing::syntax_definition::ContextId;
...
pub struct SyntaxSet {
    ...
    #[serde(skip)]
    pub first_line_cache: AtomicLazyCell<FirstLineCache>,
}
...
impl std::fmt::Debug for SyntaxReference {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("SyntaxReference {")?;
        f.write_str(format!("  name: {:?}.to_string(),\n", self.name).as_str())?;
        f.write_str(
            format!(
                "  file_extensions: vec![{}],\n",
        ...
        f.write_str(format!("  hidden: {:?},\n", self.hidden).as_str())?;

        if self.variables.is_empty() {
            f.write_str(format!("  variables: std::collections::HashMap::new(),\n").as_str())?;
        } else {
            f.write_str(
        ...
        if self.contexts.is_empty() {
            f.write_str(format!("  contexts: std::collections::HashMap::new(),\n").as_str())?;
        } else {
            f.write_str(
                format!("  contexts: {{\n    let mut v = std::collections::HashMap::new();\n")
                    .as_str(),
            )?;
            for (n, v) in self.contexts.iter() {
                f.write_str(format!("    v.insert({:?}.to_string(), {:?});\n", n, v).as_str())?;
            }
            f.write_str(format!("    v\n  }}\n").as_str())?;
        }
