...
use crate::syntect::parsing::syntax_set::{SyntaxReference, SyntaxSet};
use fnv::FnvHasher;
...
impl std::fmt::Debug for ParseState {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("ParseState {\n")?;
        f.write_str(format!("stack: vec!{:#?},\n", self.stack).as_str())?;
        f.write_str(format!("first_line: {:#?},\n", self.first_line).as_str())?;
        f.write_str(format!("proto_starts: vec!{:#?}\n", self.proto_starts).as_str())?;
        f.write_str("}")
    }
}
...
impl std::fmt::Debug for StateLevel {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("StateLevel {\n")?;
        f.write_str(format!("context: {:#?},\n", self.context).as_str())?;
        f.write_str(format!("prototypes: vec!{:#?},\n", self.prototypes).as_str())?;
        f.write_str(format!("captures: vec!{:#?}\n", self.captures).as_str())?;
        f.write_str("}")
    }
}
